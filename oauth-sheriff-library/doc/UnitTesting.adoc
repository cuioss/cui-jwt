...
This document describes the test utilities provided by the `oauth-sheriff-library` project for testing JWT validation functionality in your applications.
...
* `de.cuioss.sheriff.oauth.library.test` - Core test utilities
* `de.cuioss.sheriff.oauth.library.test.generator` - Token and claim generators
...
When your application uses the `HttpWellKnownResolver` to discover OIDC provider details, you'll want to test this integration without making actual network calls. The `de.cuioss.sheriff.oauth.library.test.dispatcher.WellKnownDispatcher` utility class is provided for this purpose.
