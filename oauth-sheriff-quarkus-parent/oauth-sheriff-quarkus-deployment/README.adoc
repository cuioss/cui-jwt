...
* `de.cuioss.sheriff.oauth.library.TokenValidator` - Main validation component
* `de.cuioss.sheriff.oauth.quarkus.config.IssuerConfigResolver` - Configuration resolver
* `de.cuioss.sheriff.oauth.library.security.SecurityEventCounter` - Security metrics
* `io.micrometer.core.instrument.MeterRegistry` - Metrics registry

**Configuration Classes (Methods Only):**

* `de.cuioss.sheriff.oauth.library.IssuerConfig` - Issuer configuration
* `de.cuioss.sheriff.oauth.library.ParserConfig` - Parser settings
* `de.cuioss.sheriff.oauth.library.jwks.http.HttpJwksLoaderConfig` - JWKS loader configuration

**Runtime Initialization:**

* `de.cuioss.sheriff.oauth.library.jwks.http.HttpJwksLoader` (runtime initialized for native image compatibility)
