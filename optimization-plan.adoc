...
1. **Add Missing Metrics in JWT Validation Library** ✅ **IN PROGRESS**:
   - Add TOKEN_FORMAT_CHECK measurement for empty/blank validation
   - Add ISSUER_EXTRACTION measurement for issuer claim extraction
   - Add ISSUER_CONFIG_RESOLUTION measurement for config lookup
   - Add TOKEN_BUILDING measurement for token object creation
   - Track token type (access/id/refresh) in metrics
...
3. **Add HTTP-Level Metrics in Quarkus Module** 🚀 **NEXT TASK**:
   - Implement HTTP request/response processing time metrics
   - Measure token extraction from Authorization header
   - Track response formatting and serialization time
   - Capture framework overhead (JAX-RS, filters, interceptors)
   - Monitor memory allocation and GC impact
   - Implementation location: cui-jwt-quarkus module
