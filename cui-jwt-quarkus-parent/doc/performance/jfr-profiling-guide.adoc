...
**Container JFR Configuration (Maven Profiles)**:

[source,bash]
----
# Build JFR-enabled image using Maven profile
mvn clean package -pl cui-jwt-quarkus-parent/cui-jwt-quarkus-integration-tests -Pdocker-jfr

# Run with JFR recording (UBI-based image has JFR built into entrypoint)
docker compose up -d

# JFR recording automatically starts and saves to /tmp/jfr-output/jwt-distro-profile.jfr
...
1. **Maven Profile Solution**: Dual-image approach with `docker-distroless` and `docker-jfr` profiles
2. **UBI-based Image for JFR**: Full JFR support with writable filesystem (187MB image)
...
[source,bash]
----
# Production build (distroless, no JFR file generation)
mvn clean package -Pdocker-distroless

# Profiling build (UBI-based, full JFR support)
