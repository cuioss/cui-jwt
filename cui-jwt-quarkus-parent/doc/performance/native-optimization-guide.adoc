...
**JFR Native Image Implementation** (Successfully Implemented):
...
[source,dockerfile]
----
# Multi-stage Dockerfile with JFR support
...
[source,bash]
----
# Configure JFR via environment variables
...
# Extract JFR recordings
docker cp container:/tmp/jfr-output/jwt-profile.jfr ./
----
...
**Key Performance Indicators** (with JFR support):

* Native image size: ~104MB (JFR-enabled distroless container)
* Startup time: 0.263s (minimal JFR overhead)
* Memory usage: ~50MB RSS (production-ready)
