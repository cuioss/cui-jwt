...
The extension provides optional default claim mappers for seamless integration with Keycloak's standard token structure. These mappers are disabled by default to maintain backward compatibility.

[cols="2,1,5"]
|===
|Property |Type |Description

|cui.jwt.keycloak.mappers.default-roles.enabled
|boolean (default: false)
...
|cui.jwt.keycloak.mappers.default-groups.enabled
|boolean (default: false)
