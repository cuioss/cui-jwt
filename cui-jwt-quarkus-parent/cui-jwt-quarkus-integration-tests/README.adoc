...
# JVM development build
docker compose build --build-arg DOCKERFILE=Dockerfile.jvm
----

**Image Characteristics**:

* **Distroless**: Minimal production image with fast startup (~0.2s)
* **JFR-enabled**: Full profiling support with debugging tools (~0.2s startup)
* **JVM**: Development image with full JVM capabilities
