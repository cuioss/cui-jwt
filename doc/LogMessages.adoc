...
|JWTValidation-146 |JWKS |Failed to parse JWKS content: %s |Logged when JWKS content parsing fails due to IO error
|JWTValidation-147 |JWKS |Invalid JWKS JSON structure: %s |Logged when JWKS JSON structure is invalid or malformed
|===
