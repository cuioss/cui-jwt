...
* **BenchmarkRunner** - Unified benchmark execution orchestration
...
import de.cuioss.benchmarking.common.runner.BenchmarkRunner;

// Build configuration from system properties
BenchmarkConfiguration config = BenchmarkConfiguration
    .fromSystemProperties()
    .build();

// Use configuration in runner
...
TokenRepository repository = TokenRepository.getSharedInstance();
...
* Reorganized into logical sub-packages for better maintainability
* Added HTTP client factory with caching and SSL support
* Moved metrics collection interfaces from integration module
* Added token repository for JWT benchmark support
* Enhanced configuration with builder pattern
* Improved artifact generation with quality gates