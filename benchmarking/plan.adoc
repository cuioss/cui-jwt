...
* [x] Audit current JSON processing usage across modules
* [x] Identify GSON features that can replace custom implementations
* [x] Optimize `JsonSerializationHelper` to use GSON features more effectively
* [x] Leverage GSON's built-in type adapters and serializers
* [x] Configure GSON instances for optimal performance (singleton pattern)
* [x] Add custom GSON type adapters/serializers as needed
* [x] Remove redundant JSON utility methods
* [x] Update all JSON processing tests
...
* [x] Run `./mvnw -Ppre-commit clean install -pl benchmarking/benchmark-library`
* [x] Run `./mvnw -Ppre-commit clean install -pl benchmarking/benchmark-integration-quarkus`
* [x] Verify JSON output format remains compatible
* [x] Test JSON round-trip serialization
* [x] Check performance of JSON processing
* [x] Commit with message: `refactor: D2. Standardize JSON Processing with GSON`
