#!/bin/bash
# Health check benchmark runner with metadata embedding
# This script runs the health check benchmark and embeds metadata for Prometheus integration
...
wrk \
    ...
    --timeout "$WRK_TIMEOUT" \
    --latency \
    -s "$SCRIPT_DIR/health_check.lua" \
    "$SERVICE_URL/q/health"
