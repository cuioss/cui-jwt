...
TokenValidator validatorNoCache = TokenValidator.builder()
        .issuerConfig(issuerConfig)
        .cacheConfig(AccessTokenCacheConfig.disabled())  // No caching
        .build();
----

NOTE: Setting `maxSize` to 0 or using `AccessTokenCacheConfig.disabled()` disables the cache completely.
