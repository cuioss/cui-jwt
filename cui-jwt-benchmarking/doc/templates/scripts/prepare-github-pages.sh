...
# Copy all page templates to root directory (same level)
cp "$TEMPLATES_DIR/index-visualizer.html" "$OUTPUT_DIR/index.html"
echo "Copied micro benchmarks template as index.html"

if [ -f "$TEMPLATES_DIR/integration-index.html" ]; then
  cp "$TEMPLATES_DIR/integration-index.html" "$OUTPUT_DIR/integration.html"
  echo "Copied integration benchmarks template as integration.html"
fi
...
if [ -f "$TEMPLATES_DIR/step-metrics-visualizer.html" ]; then
  ...
  echo "Copied step metrics visualizer template as step-metrics.html"
fi

if [ -f "$TEMPLATES_DIR/performance-trends.html" ]; then
  cp "$TEMPLATES_DIR/performance-trends.html" "$OUTPUT_DIR/trends.html"
  echo "Copied performance trends template as trends.html"
fi
